...
class MinelogyTest:
    ...
    def test_minelogy(self):
        ...
        print(f"(how-get cobblestone) : {output}") # '[(do-mine ((: stone type) (: stone variant)))]'
        output = utils.run('!(how-get stick)')[0]
        print(f"(how-get stick) : {output}") #'(do-craft ((: planks type) (: $x variant) (: 2 quantity)))'
    ...
    def test_minelogy_wtypes(self):
        ...
        print(kb.run('''
            !(mine (CBlockV log oak) $_) ; (CEntityV log oak)
            !(craft (list ((CEntityV log oak) $_))) ; ((CEntityV planks oak) 4)
            ''', flat=True))
        ...
        print(utils.run('''
            !(get-mine-block log oak) ; (CBlockV log oak)
            !(get-mine-block cobblestone) ; (CBlockV stone stone)
            !(get-ingredients planks oak) ; (list ((CEntityV log oak) 1))
            ''', flat=True),
        )
        output = utils.run('!(get-ingredients wooden_pickaxe)')[0]
        print(output) #'(list ((CEntityT stick) 2) ((CEntityV planks $_) 3))'


if __name__ == "__main__":
    MinelogyTest().test_minelogy()
    MinelogyTest().test_minelogy_wtypes()
