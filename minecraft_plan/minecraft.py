...
class MinecraftTest:
    ...
    def test_minecraft_planning(self):
        ...
        metta.run('''
            ...
            (= (pack $n cobblestones)
               (if (> $n 0) (allof (cobblestone) (pack (- $n 1) cobblestones)) nop))

            (= (stone-pickaxe)
               (craft stone-pickaxe (crafting-table)
                 (allof (pack 3 cobblestones) (pack 2 sticks))))
        ''')

        assert S('wooden-pickaxe') not in inventory
        metta.run('!(wooden-pickaxe)')
        assert S('four-planks') in inventory
        assert S('crafting-table') in inventory
        assert S('wooden-pickaxe') in inventory
    ...
    def test_minecraft_planning_with_abstractions(self):
        ...
        metta.run('''
            ...
            (= (make $x)
               (if (and ($x made-from $comp) ($x made-at $tool))
                   (, (get $tool) (get $comp) (do-make $x $tool $comp)) (empty)))

            (= (mine $x)
               (if (and ($x mined-using $tool) ($x mined-from $source))
                   (, (get $tool) (find $source) (do-mine $x $source $tool)) (empty)))

            (= (get $x)
               (if (and (not (in-inventory $x))
                   (can-be-mined $x))
                   (mine $x) (empty)))
            (= (get $x)
               (if (and (not (in-inventory $x))
...
if __name__ == "__main__":
    MinecraftTest().test_minecraft_planning()
    MinecraftTest().test_minecraft_planning_with_abstractions()
